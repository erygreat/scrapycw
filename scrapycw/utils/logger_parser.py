    ...
    def handle_date_pattern(self):
        ...
        for value in self.NOT_SUPPORT_PATTERNS:
            if date_pattern.find(value) != -1:
                raise ScrapycwLoggerParserException("不支持日期格式: {}".format(value))
...
class LoggerParser:
    ...
    def get_file_size(self):
        ...
        if not os.path.exists(self.filename):
            raise ScrapycwLoggerParserException("日志文件不存在")
        # 文件是否超过最大解析大小
    ...
    def read_log_file(self):
        if self.log_size > HANDLE_LOG_MAXIMUM_SIZE:
            ...
            raise ScrapycwLoggerParserException(
                "当前设置最大可解析日志大小为{}，当前日志大小为: {}, 可以修改 HANDLE_LOG_MAXIMUM_SIZE 修改可解析日志大小".format(max_size, current_size))
    ...
    def parse(self):
        ...
        for handler_class in self.handler_classes:
            ...
            logger_pattern = logger_pattern.replace(re.escape(handler.format), "({})".format(handler.pattern), -1)
            indexs = [m.start() for m in re.finditer(re.escape(handler.format), self.log_format)]
            for index in indexs:
...
class ScrapyLoggerParser(LoggerParser):
    ...
    def _parse_logs(self, logs):
        ...
        if end_line.get("relativeCreated", None):
            relative_created = float(end_line.get("relativeCreated", None)) / 1000
            continuous_time = datetime.datetime.fromtimestamp(relative_created) - datetime.datetime.fromtimestamp(0)
        ...
        return {
            ...
            "pages": crawl_stats[-1]['page_count'] if len(crawl_stats) > 0 else None,
            "items": crawl_stats[-1]['item_count'] if len(crawl_stats) > 0 else None,
