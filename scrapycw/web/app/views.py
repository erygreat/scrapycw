...
class View:
    ...
    def spiders(request):
        return HttpResponse(json.dumps(SpiderService.list()), content_type='application/json')
    ...
    def crawl(request):
        ...
        project = body.get("project", SCRAPY_DEFAULT_PROJECT)
        spname = body.get("spider")

        result = SpiderService.run(project=project, cmdline_settings=settings, spname=spname, spargs=spargs)
        return HttpResponse(json.dumps(result), content_type='application/json')
